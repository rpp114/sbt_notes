...
def user_tasks():
	...
	if current_user.role_id == 4:
		...
		notes_needing_approval = models.ClientApptNote.query.filter(models.ClientApptNote.approved == False, 
																	...
																	or_(models.ClientApptNote.encrypted_note == None, models.ClientApptNote.encrypted_note != empty_bytes))\
		...
		signatures_needed_query = text (f'''SELECT client_appt.id, client.first_name, client.last_name, client_appt.start_datetime
								...
								and client_appt.cancelled = 0
								and client.regional_center_id > 1
...
def billing_appt():
	...
	rcs = models.RegionalCenter.query.filter(models.RegionalCenter.company_id == company_id).order_by(desc(models.RegionalCenter.id)).all()
